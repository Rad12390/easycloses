...
namespace LocalsBest\MedAppUserBundle\Controller;
use LocalsBest\MedAppUserBundle\Entity\User;
use LocalsBest\MedAppUserBundle\Form\ForgotType;
...
class AuthController extends Controller
{
    ...
    public function loginAction(Request $request, $slug =null)
    {
        ...
        if (isset($_SERVER['HTTP_USER_AGENT']) && preg_match('/msie ([2-9]|10)/i', $_SERVER['HTTP_USER_AGENT'])) {
            return $this->render('@LocalsBestMedAppUser/auth/ie_message.html.twig');
        }
        ...
        if ($lastUsername !== '') {
            ...
            $user = $em->getRepository('LocalsBestMedAppUserBundle:User')->findOneBy(['username' => $lastUsername]);
...
//                    $em->persist($business);
    ...
    public function forgotPasswordAction(Request $request)
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if ($form->isValid()) {
                ...
                $user = $em->getRepository('LocalsBest\MedAppUserBundle\Entity\User')->findOneBy(['username' => $username]);
                ...
                if ($user) {
                    ...
                    // Update User Entity
                    $em->getRepository('LocalsBest\MedAppUserBundle\Entity\User')->save($user);
        ...
        return $this->render('@LocalsBestMedAppUser/auth/forgotPassword.html.twig', array('form' => $form->createView(),
            'error' => $error)
        );
    }
    ...
    public function resetPasswordAction(Request $request, $token)
    {
        ...
        $user = $this->getDoctrine()->getRepository('LocalsBest\MedAppUserBundle\Entity\User')
        ...
        if ($registrationTime >= $validRegistrationTime) {
            ...
            if ($request->getMethod() == 'POST') {
                ...
                if ($form->isValid()) {
                    ...
                    // Save User Entity
                    $this->getDoctrine()->getManager()->getRepository('LocalsBest\MedAppUserBundle\Entity\User')->save($user);
                    // Show flash message
